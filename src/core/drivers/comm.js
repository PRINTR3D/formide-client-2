...
function comm() {
    ...
    driver.on('exit', function (code, signal) {
        Globals.log(`[drivers] - Process exiting because driver fork crashed, ${code}, ${signal}`, 1, 'error');
        process.exit(code);
    });

    // handle driver process error
    driver.on('error', function (err) {
        Globals.log(err, 1, 'warn');
    });
    ...
    function on(callback) {
        driver.on('message', function (message) {
            if (!message.type)
                ...
                Globals.log('[drivers] - Formide driver started successfully in separate thread', 1, 'info');
            else if (message.type === 'error' && message.data)
                Globals.log(`[drivers] - Driver error: ${message.data}`, 1, 'warn');
